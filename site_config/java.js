export default {
    'en-us': {
        sidemenu: [
            {
                title: 'Java 8 新特性',
                children: [
                    {
                        title: '特性一览',
                        link: '/zh-cn/docs/java/java8/SUMMARY.html',
                    },
                    {
                        title: 'Lambda表达式',
                        link: '/zh-cn/docs/java/java8/Lambda.html',
                    },
                    {
                        title: '函数式接口',
                        link: '/zh-cn/docs/java/java8/FunctionInterface.html',
                    },
                    {
                        title: '方法引用与构造器引用',
                        link: '/zh-cn/docs/java/java8/Method.html',
                    },
                    {
                        title: 'Stream API',
                        link: '/zh-cn/docs/java/java8/Stream.html',
                    },
                    {
                        title: 'Optional容器',
                        link: '/zh-cn/docs/java/java8/Optional.html',
                    },
                    {
                        title: '接口中的默认方法与静态方法',
                        link: '/zh-cn/docs/java/java8/Interface.html',
                    },
                    {
                        title: '新时间日期API',
                        link: '/zh-cn/docs/java/java8/Date.html',
                    },
                    {
                        title: '其他新特性',
                        link: '/zh-cn/docs/java/java8/Other.html',
                    },
                ],
            },
            {
                title: 'Java 9 新特性',
                children: [
                    {
                        title: '特性一览',
                        link: '/zh-cn/docs/java/java9/SUMMARY.html',
                    },
                    {
                        title: '模块化系统',
                        link: '/zh-cn/docs/java/java9/jigsaw.html',
                    },
                    {
                        title: 'Jshell',
                        link: '/zh-cn/docs/java/java9/jshell.html',
                    },
                    {
                        title: '多版本兼容jar包',
                        link: '/zh-cn/docs/java/java9/MultiRelease.html',
                    },
                    {
                        title: '语法改进',
                        link: '/zh-cn/docs/java/java9/Grammar.html',
                    },
                    {
                        title: 'String存储结构变更',
                        link: '/zh-cn/docs/java/java9/String.html',
                    },
                    {
                        title: '集合工厂方法',
                        link: '/zh-cn/docs/java/java9/CollectionFactory.html',
                    },
                    {
                        title: '增强的 StreamAPI',
                        link: '/zh-cn/docs/java/java9/Stream.html',
                    },
                    {
                        title: '多分辨率图像API',
                        link: '/zh-cn/docs/java/java9/MultiResolutionImages.html',
                    },
                    {
                        title: '全新的HTTP客户端API',
                        link: '/zh-cn/docs/java/java9/HttpClient.html',
                    },
                    {
                        title: 'Deprecated的相关API',
                        link: '/zh-cn/docs/java/java9/DeprecatedAPI.html',
                    },
                    {
                        title: '智能Java编译工具',
                        link: '/zh-cn/docs/java/java9/JavaCompiler.html',
                    },
                    {
                        title: '统一的JVM日志系统',
                        link: '/zh-cn/docs/java/java9/UnifiedJVMLogging.html',
                    },
                    {
                        title: 'javadoc的HTML5支持',
                        link: '/zh-cn/docs/java/java9/JavaDoc.html',
                    },
                    {
                        title: 'Javascript引擎升级：Nashorn',
                        link: '/zh-cn/docs/java/java9/Nashorn.html',
                    },
                    {
                        title: 'java的动态编译器',
                        link: '/zh-cn/docs/java/java9/DynamicCompilation.html',
                    },
                    {
                        title: '进程 API',
                        link: '/zh-cn/docs/java/java9/ProcessAPI.html',
                    },
                    {
                        title: '平台日志 API 和 服务',
                        link: '/zh-cn/docs/java/java9/PlatformLoggingAPIandService.html',
                    },
                    {
                        title: '反应式流',
                        link: '/zh-cn/docs/java/java9/Reactor.html',
                    },
                    {
                        title: '变量句柄',
                        link: '/zh-cn/docs/java/java9/VariableHandles.html',
                    },
                    {
                        title: '并发',
                        link: '/zh-cn/docs/java/java9/Concurrency.html',
                    },
                    {
                        title: 'I/O 流新特性',
                        link: '/zh-cn/docs/java/java9/IO.html',
                    },
                    {
                        title: '改进应用安全性能',
                        link: '/zh-cn/docs/java/java9/SHA-3HashAlgorithms.html',
                    },
                    {
                        title: '用户界面',
                        link: '/zh-cn/docs/java/java9/Desktop.html',
                    },
                ],
            },
            {
                title: 'Java 10 新特性',
                children: [
                    {
                        title: '特性一览',
                        link: '/zh-cn/docs/java/java10/SUMMARY.html',
                    },
                    {
                        title: '局部变量类型推断',
                        link: '/zh-cn/docs/java/java10/Local-VariableTypeInference.html',
                    },
                    {
                        title: '整合JDK代码仓库',
                        link: '/zh-cn/docs/java/java10/ConsolidateJDKForest.html',
                    },
                    {
                        title: '统一的垃圾回收接口',
                        link: '/zh-cn/docs/java/java10/Garbage-CollectorInterface.html',
                    },
                    {
                        title: '并行全垃圾回收器G1',
                        link: '/zh-cn/docs/java/java10/G1.html',
                    },
                    {
                        title: '应用程序类数据共享',
                        link: '/zh-cn/docs/java/java10/ApplicationClassDataSharing.html',
                    },
                    {
                        title: '线程-局部管控',
                        link: '/zh-cn/docs/java/java10/ThreadLocalHandshakes.html',
                    },
                    {
                        title: '移除Native-Header自动生成工具',
                        link: '/zh-cn/docs/java/java10/NativeHeader.html',
                    },
                    {
                        title: '额外的Unicode语言标签扩展',
                        link: '/zh-cn/docs/java/java10/Unicode.html',
                    },
                    {
                        title: '备用存储装置上的堆分配',
                        link: '/zh-cn/docs/java/java10/HeapAllocation.html',
                    },
                    {
                        title: '基于Java的实验性JIT编译器',
                        link: '/zh-cn/docs/java/java10/JIT.html',
                    },
                    {
                        title: '根证书认证',
                        link: '/zh-cn/docs/java/java10/RootCertificates.html',
                    },
                    {
                        title: '基于时间的版本发布模式',
                        link: '/zh-cn/docs/java/java10/ReleaseVersioning.html',
                    },
                ],
            },
            {
                title: 'Java 11 新特性',
                children: [
                    {
                        title: '特性一览',
                        link: '/zh-cn/docs/java/java11/SUMMARY.html',
                    },
                    {
                        title: '基于嵌套的访问控制',
                        link: '/zh-cn/docs/java/java11/AccessControl.html',
                    },
                    {
                        title: '动态类文件常量',
                        link: '/zh-cn/docs/java/java11/DynamicClassFile.html',
                    },
                    {
                        title: 'Epsilon：低开销垃圾回收器',
                        link: '/zh-cn/docs/java/java11/Epsilon.html',
                    },
                    {
                        title: '删除 JavaEE 和 CORBA 模块',
                        link: '/zh-cn/docs/java/java11/RemoveJavaEEAndCORBA.html',
                    },
                    {
                        title: '标准 HTTP Client',
                        link: '/zh-cn/docs/java/java11/HTTPClient.html',
                    },
                    {
                        title: '用于 Lambda 参数的局部变量语法',
                        link: '/zh-cn/docs/java/java11/Lambda.html',
                    },
                    {
                        title: 'Unicode 10',
                        link: '/zh-cn/docs/java/java11/Unicode.html',
                    },
                    {
                        title: 'ChaCha20 和 Poly1305 加密算法',
                        link: '/zh-cn/docs/java/java11/CryptographicAlgorithms.html',
                    },
                    {
                        title: '飞行记录器',
                        link: '/zh-cn/docs/java/java11/FlightRecorder.html',
                    },
                    {
                        title: '简化启动单个源代码文件',
                        link: '/zh-cn/docs/java/java11/LaunchSingleFile.html',
                    },
                    {
                        title: '低开销的 HeapProfiling',
                        link: '/zh-cn/docs/java/java11/HeapProfiling.html',
                    },
                    {
                        title: '支持 TLS 1.3 协议',
                        link: '/zh-cn/docs/java/java11/TLS.html',
                    },
                    {
                        title: 'ZGC：可伸缩低延迟垃圾收集器',
                        link: '/zh-cn/docs/java/java10/ZGC.html',
                    },
                    {
                        title: '废除Nashorn javascript引擎',
                        link: '/zh-cn/docs/java/java11/Nashorn.html',
                    },
                    {
                        title: '废除 Pack200',
                        link: '/zh-cn/docs/java/java11/Pack200.html',
                    },
                ],
            },
            {
                title: 'Java 12 新特性',
                children: [
                    {
                        title: '特性一览',
                        link: '/zh-cn/docs/java/java12/SUMMARY.html',
                    },
                    {
                        title: 'Shenandoah：一个低停顿垃圾收集器（实验阶段）',
                        link: '/zh-cn/docs/java/java12/Shenandoah.html',
                    },
                    {
                        title: 'Switch 表达式扩展',
                        link: '/zh-cn/docs/java/java12/Switch.html',
                    },
                    {
                        title: '微基准套件',
                        link: '/zh-cn/docs/java/java12/JMH.html',
                    },
                    {
                        title: '引入 JVM 常量 API',
                        link: '/zh-cn/docs/java/java12/jvm.html',
                    },
                    {
                        title: '改进 AArch64 实现',
                        link: '/zh-cn/docs/java/java12/AArch64.html',
                    },
                    {
                        title: '默认类数据共享（CDS）存档',
                        link: '/zh-cn/docs/java/java12/CDS.html',
                    },
                    {
                        title: '可中止的G1 Mixed GC',
                        link: '/zh-cn/docs/java/java12/G1.html',
                    },
                    {
                        title: 'G1及时返回未使用的已分配内存',
                        link: '/zh-cn/docs/java/java12/G11.html',
                    },
                    {
                        title: '其他特性',
                        link: '/zh-cn/docs/java/java12/other.html',
                    },
                ],
            },
            {
                title: 'Java 13 新特性',
                children: [
                    {
                        title: '特性一览',
                        link: '/zh-cn/docs/java/java13/SUMMARY.html',
                    },
                    {
                        title: 'switch表达式（预览）',
                        link: '/zh-cn/docs/java/java13/switch.html',
                    },
                    {
                        title: '文本块（预览）',
                        link: '/zh-cn/docs/java/java13/block.html',
                    },
                    {
                        title: '动态CDS档案（动态类数据共享归档）',
                        link: '/zh-cn/docs/java/java13/cds.html',
                    },
                    {
                        title: 'ZGC:取消使用未使用的内存 ',
                        link: '/zh-cn/docs/java/java13/jvm.html',
                    },
                    {
                        title: '重新实现旧版套接字API',
                        link: '/zh-cn/docs/java/java13/socket.html',
                    },
                    {
                        title: '其他特性',
                        link: '/zh-cn/docs/java/java13/other.html',
                    },
                ],
            },
        ],
        barText: 'Java 8 & 9 & 10 & 11 & 12 & 13新特性',
    },
    'zh-cn': {
        sidemenu: [
            {
                title: 'Java 8 新特性',
                children: [
                    {
                        title: 'Java 8特性一览',
                        link: '/zh-cn/docs/java/java8/SUMMARY.html',
                    },
                    {
                        title: 'Lambda表达式',
                        link: '/zh-cn/docs/java/java8/Lambda.html',
                    },
                    {
                        title: '函数式接口',
                        link: '/zh-cn/docs/java/java8/FunctionInterface.html',
                    },
                    {
                        title: '方法引用与构造器引用',
                        link: '/zh-cn/docs/java/java8/Method.html',
                    },
                    {
                        title: 'Stream API',
                        link: '/zh-cn/docs/java/java8/Stream.html',
                    },
                    {
                        title: 'Optional容器',
                        link: '/zh-cn/docs/java/java8/Optional.html',
                    },
                    {
                        title: '接口中的默认方法与静态方法',
                        link: '/zh-cn/docs/java/java8/Interface.html',
                    },
                    {
                        title: '新时间日期API',
                        link: '/zh-cn/docs/java/java8/Date.html',
                    },
                    {
                        title: 'Java 8 其他新特性',
                        link: '/zh-cn/docs/java/java8/Other.html',
                    },
                ],
            },
            {
                title: 'Java 9 新特性',
                children: [
                    {
                        title: 'Java 9 特性一览',
                        link: '/zh-cn/docs/java/java9/SUMMARY.html',
                    },
                    {
                        title: '模块化系统',
                        link: '/zh-cn/docs/java/java9/jigsaw.html',
                    },
                    {
                        title: 'Jshell',
                        link: '/zh-cn/docs/java/java9/jshell.html',
                    },
                    {
                        title: '多版本兼容jar包',
                        link: '/zh-cn/docs/java/java9/MultiRelease.html',
                    },
                    {
                        title: '语法改进',
                        link: '/zh-cn/docs/java/java9/Grammar.html',
                    },
                    {
                        title: 'String存储结构变更',
                        link: '/zh-cn/docs/java/java9/String.html',
                    },
                    {
                        title: '集合工厂方法',
                        link: '/zh-cn/docs/java/java9/CollectionFactory.html',
                    },
                    {
                        title: '增强的 StreamAPI',
                        link: '/zh-cn/docs/java/java9/Stream.html',
                    },
                    {
                        title: '多分辨率图像API',
                        link: '/zh-cn/docs/java/java9/MultiResolutionImages.html',
                    },
                    {
                        title: '全新的HTTP客户端API',
                        link: '/zh-cn/docs/java/java9/HttpClient.html',
                    },
                    {
                        title: 'Deprecated的相关API',
                        link: '/zh-cn/docs/java/java9/DeprecatedAPI.html',
                    },
                    {
                        title: '智能Java编译工具',
                        link: '/zh-cn/docs/java/java9/JavaCompiler.html',
                    },
                    {
                        title: '统一的JVM日志系统',
                        link: '/zh-cn/docs/java/java9/UnifiedJVMLogging.html',
                    },
                    {
                        title: 'javadoc的HTML5支持',
                        link: '/zh-cn/docs/java/java9/JavaDoc.html',
                    },
                    {
                        title: 'Javascript引擎升级：Nashorn',
                        link: '/zh-cn/docs/java/java9/Nashorn.html',
                    },
                    {
                        title: 'java的动态编译器',
                        link: '/zh-cn/docs/java/java9/DynamicCompilation.html',
                    },
                    {
                        title: '进程 API',
                        link: '/zh-cn/docs/java/java9/ProcessAPI.html',
                    },
                    {
                        title: '平台日志 API 和 服务',
                        link: '/zh-cn/docs/java/java9/PlatformLoggingAPIandService.html',
                    },
                    {
                        title: '反应式流',
                        link: '/zh-cn/docs/java/java9/Reactor.html',
                    },
                    {
                        title: '变量句柄',
                        link: '/zh-cn/docs/java/java9/VariableHandles.html',
                    },
                    {
                        title: '并发',
                        link: '/zh-cn/docs/java/java9/Concurrency.html',
                    },
                    {
                        title: 'I/O 流新特性',
                        link: '/zh-cn/docs/java/java9/IO.html',
                    },
                    {
                        title: '改进应用安全性能',
                        link: '/zh-cn/docs/java/java9/SHA-3HashAlgorithms.html',
                    },
                    {
                        title: '用户界面',
                        link: '/zh-cn/docs/java/java9/Desktop.html',
                    },
                ],
            },
            {
                title: 'Java 10 新特性',
                children: [
                    {
                        title: 'Java 10 特性一览',
                        link: '/zh-cn/docs/java/java10/SUMMARY.html',
                    },
                    {
                        title: '局部变量类型推断',
                        link: '/zh-cn/docs/java/java10/Local-VariableTypeInference.html',
                    },
                    {
                        title: '整合JDK代码仓库',
                        link: '/zh-cn/docs/java/java10/ConsolidateJDKForest.html',
                    },
                    {
                        title: '统一的垃圾回收接口',
                        link: '/zh-cn/docs/java/java10/Garbage-CollectorInterface.html',
                    },
                    {
                        title: '并行全垃圾回收器G1',
                        link: '/zh-cn/docs/java/java10/G1.html',
                    },
                    {
                        title: '应用程序类数据共享',
                        link: '/zh-cn/docs/java/java10/ApplicationClassDataSharing.html',
                    },
                    {
                        title: '线程-局部管控',
                        link: '/zh-cn/docs/java/java10/ThreadLocalHandshakes.html',
                    },
                    {
                        title: '移除Native-Header自动生成工具',
                        link: '/zh-cn/docs/java/java10/NativeHeader.html',
                    },
                    {
                        title: '额外的Unicode语言标签扩展',
                        link: '/zh-cn/docs/java/java10/Unicode.html',
                    },
                    {
                        title: '备用存储装置上的堆分配',
                        link: '/zh-cn/docs/java/java10/HeapAllocation.html',
                    },
                    {
                        title: '基于Java的实验性JIT编译器',
                        link: '/zh-cn/docs/java/java10/JIT.html',
                    },
                    {
                        title: '根证书认证',
                        link: '/zh-cn/docs/java/java10/RootCertificates.html',
                    },
                    {
                        title: '基于时间的版本发布模式',
                        link: '/zh-cn/docs/java/java10/ReleaseVersioning.html',
                    },
                ],
            },
            {
                title: 'Java 11 新特性',
                children: [
                    {
                        title: 'Java 11 特性一览',
                        link: '/zh-cn/docs/java/java11/SUMMARY.html',
                    },
                    {
                        title: '基于嵌套的访问控制',
                        link: '/zh-cn/docs/java/java11/AccessControl.html',
                    },
                    {
                        title: '动态类文件常量',
                        link: '/zh-cn/docs/java/java11/DynamicClassFile.html',
                    },
                    {
                        title: 'Epsilon：低开销垃圾回收器',
                        link: '/zh-cn/docs/java/java11/Epsilon.html',
                    },
                    {
                        title: '删除 JavaEE 和 CORBA 模块',
                        link: '/zh-cn/docs/java/java11/RemoveJavaEEAndCORBA.html',
                    },
                    {
                        title: '标准 HTTP Client',
                        link: '/zh-cn/docs/java/java11/HTTPClient.html',
                    },
                    {
                        title: '用于 Lambda 参数的局部变量语法',
                        link: '/zh-cn/docs/java/java11/Lambda.html',
                    },
                    {
                        title: 'Unicode 10',
                        link: '/zh-cn/docs/java/java11/Unicode.html',
                    },
                    {
                        title: 'ChaCha20 和 Poly1305 加密算法',
                        link: '/zh-cn/docs/java/java11/CryptographicAlgorithms.html',
                    },
                    {
                        title: '飞行记录器',
                        link: '/zh-cn/docs/java/java11/FlightRecorder.html',
                    },
                    {
                        title: '简化启动单个源代码文件',
                        link: '/zh-cn/docs/java/java11/LaunchSingleFile.html',
                    },
                    {
                        title: '低开销的 HeapProfiling',
                        link: '/zh-cn/docs/java/java11/HeapProfiling.html',
                    },
                    {
                        title: '支持 TLS 1.3 协议',
                        link: '/zh-cn/docs/java/java11/TLS.html',
                    },
                    {
                        title: 'ZGC：可伸缩低延迟垃圾收集器',
                        link: '/zh-cn/docs/java/java10/ZGC.html',
                    },
                    {
                        title: '废除Nashorn javascript引擎',
                        link: '/zh-cn/docs/java/java11/Nashorn.html',
                    },
                    {
                        title: '废除 Pack200',
                        link: '/zh-cn/docs/java/java11/Pack200.html',
                    },
                ],
            },
            {
                title: 'Java 12 新特性',
                children: [
                    {
                        title: 'Java 12 特性一览',
                        link: '/zh-cn/docs/java/java12/SUMMARY.html',
                    },
                    {
                        title: 'Shenandoah：一个低停顿垃圾收集器（实验阶段）',
                        link: '/zh-cn/docs/java/java12/Shenandoah.html',
                    },
                    {
                        title: 'Switch 表达式扩展',
                        link: '/zh-cn/docs/java/java12/Switch.html',
                    },
                    {
                        title: '微基准套件',
                        link: '/zh-cn/docs/java/java12/JMH.html',
                    },
                    {
                        title: '引入 JVM 常量 API',
                        link: '/zh-cn/docs/java/java12/jvm.html',
                    },
                    {
                        title: '改进 AArch64 实现',
                        link: '/zh-cn/docs/java/java12/AArch64.html',
                    },
                    {
                        title: '默认类数据共享（CDS）存档',
                        link: '/zh-cn/docs/java/java12/CDS.html',
                    },
                    {
                        title: '可中止的G1 Mixed GC',
                        link: '/zh-cn/docs/java/java12/G1.html',
                    },
                    {
                        title: 'G1及时返回未使用的已分配内存',
                        link: '/zh-cn/docs/java/java12/G11.html',
                    },
                    {
                        title: 'Java 12 其他特性',
                        link: '/zh-cn/docs/java/java12/other.html',
                    },
                ],
            },
            {
                title: 'Java 13 新特性',
                children: [
                    {
                        title: 'Java 13 特性一览',
                        link: '/zh-cn/docs/java/java13/SUMMARY.html',
                    },
                    {
                        title: 'switch表达式（预览）',
                        link: '/zh-cn/docs/java/java13/switch.html',
                    },
                    {
                        title: '文本块（预览）',
                        link: '/zh-cn/docs/java/java13/block.html',
                    },
                    {
                        title: '动态CDS档案（动态类数据共享归档）',
                        link: '/zh-cn/docs/java/java13/cds.html',
                    },
                    {
                        title: 'ZGC:取消使用未使用的内存 ',
                        link: '/zh-cn/docs/java/java13/jvm.html',
                    },
                    {
                        title: '重新实现旧版套接字API',
                        link: '/zh-cn/docs/java/java13/socket.html',
                    },
                    {
                        title: 'Java 13 其他特性',
                        link: '/zh-cn/docs/java/java13/other.html',
                    },
                ],
            },
        ],
        barText: 'Java 8 & 9 & 10 & 11 & 12 & 13新特性',
    },
};
